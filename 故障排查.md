...
### 错误：`403 Client Error: Forbidden`（RSSHub）
**原因**：RSSHub路由访问被拒绝
**解决**：
1. **检查路由是否可用**：
   - 在浏览器中直接访问RSSHub路由
...
2. **查找替代RSS源**：
   - 使用RSSHub Radar检测网站是否有原生RSS源
   - 尝试使用其他RSSHub路由
   - 查看RSSHub文档：https://docs.rsshub.app/
...
4. **详细指南**：查看 `RSSHub故障排查.md`

### 错误：`404 Client Error: Not Found`（RSSHub）
**原因**：RSSHub路由不存在或格式错误
**解决**：
1. **检查路由格式**：
   - 访问RSSHub文档：https://docs.rsshub.app/
   - 搜索对应的平台，查看正确的路由格式
   - 常见错误：`/twitter/media/用户名` 应该是 `/twitter/user/用户名`
...
4. **使用正确的路由**：
   - 参考RSSHub文档使用正确的路由格式
   - 某些路由可能需要认证（需要自建RSSHub实例）

5. **详细指南**：查看 `RSSHub故障排查.md`
...
### 错误：`未获取到文章`（Nitter）
**原因**：Nitter实例不可用或用户名不存在
**解决**：
1. **检查Nitter实例状态**：
   - 访问：https://status.d420.de/ 查看实例状态
   - 在浏览器中访问Nitter实例，确认是否可用

2. **尝试其他Nitter实例**：
   - `https://nitter.net/用户名/rss`
   - `https://nitter.it/用户名/rss`
   - `https://nitter.pussthecat.org/用户名/rss`
   - 更多实例：https://status.d420.de/
...
4. **使用替代方案**：
   - 使用RSSHub：`https://rsshub.app/twitter/user/用户名`
   - 使用其他Twitter RSS服务

5. **详细指南**：查看 `Nitter故障排查.md`
